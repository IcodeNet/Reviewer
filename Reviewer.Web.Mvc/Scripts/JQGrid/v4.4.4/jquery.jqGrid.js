			...
			$.each($t.p.colModel,function(){
				...
				if(this.search){
					...
					case 'text':
						...
						$(thd).append("<input type='text' style='width:90%;padding:0px;' name='"+(cm.index || cm.name)+"' id='gs_"+cm.name+"' value='"+df+"'/>");
	...
	createEl : function(eltype,options,vl,autowidth, ajaxso) {
		...
		switch (eltype)
		{
			case "textarea" :
				...
				if(autowidth) {
					if(!options.cols) { $(elem).css({width:"90%"});}
			...
			case "button" :
				...
				if(eltype != "button"){
					if(autowidth) {
						if(!options.size) { $(elem).css({width:"90%"}); }
