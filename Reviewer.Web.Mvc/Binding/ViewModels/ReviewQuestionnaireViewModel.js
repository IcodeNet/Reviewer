...
(function (ns) {
    ...
    var vm = function () {
        ...
        self.loadQuestions = function (model) {
            ...
            self.viewModelHelper.apiGet('api/resources/questionrecords?questionnaireId=' + self.getParameterByName('id'),
                ...
                function (result) {
                    ...
                    if ((self.header().QuestionnaireStatus == 'Reviewed'
                        || self.header().QuestionnaireStatus == 'Rejected in Review'
                        || self.header().QuestionnaireStatus == 'Completed/Signed Off'
                        || self.header().QuestionnaireStatus == 'In Progress'
                        ) && isUserReviewer) {
        ...
        self.approve = function () {
            ...
            self.viewModelHelper.apiPost("api/operations/approveinreview",
                self.getDto(),
                function (result) {
                    toastr.success("Approved! ", "Operation Result");
                    $('#wizard').smartWizard('showMessage', 'Reviewed');
        ...
        self.postChanges = function () {
            ...
            self.viewModelHelper.apiPost("api/operations/rejectinreview", self.getDto(),
                                   function (result) {
                                       toastr.success("Rejected! ", "Operation Result");
                                       $('#wizard').smartWizard('showMessage', 'Rejected in Review');
                                       $('#wizard').smartWizard('showError', { stepnum: 1, iserror: true });
                                   },
                                   function (error) {
                                       toastr.error(error.responseText, "Operation Result");
                                   },
        ...
        self.loadGrid = function (gridId, arrayOfQuestions) {
            ...
            var myGrid = $(gridId).jqGrid({
                ...
                colModel: [
                    ...
                    { name: "Answer", width: 240, "index": "Answer", align: "center", editable: true },
